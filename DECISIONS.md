...
- Factory tests monkeypatch config.GOOGLE_API_KEY so the missing-key path is deterministic even when a real .env is present, and they pass the provider explicitly so the suite is green with LLM_PROVIDER unset. 24 tests, no network.
