...
- Moved `unique_sources` out of app.py into rag_chain. Deduping retrieved docs by term+page (keeping the closest hit) is citation logic, not layout, and while it lived in app.py it could not be tested — importing app.py runs `st.set_page_config` and the index bootstrap. Now covered for the sub-chunk case it exists for, for the same term on two pages, and for no results.
- Added `python -m src.embed_store --clean`. `delete_collection` drops the collection from Chroma's metadata db but leaves its HNSW segment directory on disk, so eight rebuilds had left eight segment dirs and 27 MB for one 3.6 MB collection. Chose removing the directory over any Chroma-level cleanup because the internals differ between the pinned 0.5.23 and the 1.5.9 this machine runs, and a filesystem delete works on both. It is a deliberate command in a fresh process, guarded to refuse while a client is open: an open client holds the sqlite file and on Windows the delete would half-finish. Verified: 8 dirs / 27 MB -> 1 dir / 3.6 MB, with the eval table and the probe distances unchanged after the rebuild.
- Closed the `json.load(open(...))` handles in chunking, eval_retrieval and eval_ragas. They relied on refcounting to close, which is a CPython implementation detail rather than a guarantee.
